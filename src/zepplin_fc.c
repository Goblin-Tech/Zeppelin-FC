...
char test_file_name[] = "0:Zepplin-FC.bin";
TCHAR file_name[17] = "0:Zepplin-FC.bin";
Ctrl_status status;
FRESULT res;
DIR dir;
FATFS fs;
...
{
	res = f_open(&log, "0:Sensor.log", FA_CREATE_ALWAYS | FA_WRITE);
	if (res) die(res);
	
	uint16_t buffer_t = 0;
	for(uint8_t i=0; i < 8; i++)
	{
		f_write(&log, &buffer_t, 2, &bw);
		buffer_t++;
	}		
}
...
{
	tick++;
	res = f_write(&log, buffer, 18, &bw);
	if (res || bw < 9)
		printf("Write Error: bw %d < 18\n", bw);
		
	uint8_t buffer_t[8];
	uint16_t pyro = 1;
	res = f_write(&log, buffer_t, 8, &bw);
	res = f_write(&log, tick, 2, &bw);
...
void Zepplin_Scan_Scripts(void)
{
...
/*
bool Zepplin_Appoge_Velocity(void)
{
	if (ZP_Accel_Y > 0 && !ZP_Accel_Y_Dir)
		ZP_APV_Stage = 1;
		
	if (ZP_Accel_Y < 0 && !ZP_Aceel_Y_Dir)
		ZP_APV_Stage = 2;
...
bool Zepplin_Appoge_Pressure(void)
{
	if (ZP_Alt_Speed < 0)
		return true; 
	return false;
